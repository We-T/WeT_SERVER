create table `users` (`email` varchar(30), `pwd` varchar(20), `phone` int(11), `name` varchar(10), primary key(email))default charset=utf8;
...
create table `trip_plan` (`index` int(10) not null auto_increment, `inherence_number` int(8), `start_day` datetime, `end_day` datetime, `area_code` varchar(20), `email` varchar(30), `present_day` datetime, primary key(`index`), foreign key (email) references users(email));
